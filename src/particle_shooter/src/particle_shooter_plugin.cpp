...
//      ignition::math::Pose3d gasStationPos  = gasStationModel->WorldPose();
      ...
      ROS_FATAL_STREAM("Load particle plugin");
      auto particleSDF = sdf::readFile("model://particle_sphere/model.sdf");
      particleSDF->Root()->GetElement("model")->GetAttribute("name")->Set("particle 1");

      _world->InsertModelSDF(*particleSDF);
